...
Proof.
  ...
  rewrite -> (H true). apply (H true).
  rewrite -> (H false). apply (H false).
  Qed.
...
Proof.
  intros.
  destruct b.
  rewrite (H true). apply (H (negb true)).
  rewrite (H false). apply (H (negb false)).
Qed.
...
Fixpoint bin_to_nat (n : bin) : nat :=
  match n with
    | x0 => 0
    | xE p => plus (bin_to_nat p) (bin_to_nat p)
    | xD p => plus 1 (plus (bin_to_nat p) (bin_to_nat p))
  end.

Lemma bin_and_nat :
  forall (n:bin), bin_to_nat (incr n) = 1 + (bin_to_nat n).
Proof.
  intros.
Example test_bin_incr1 : bin_to_nat (incr x0) =  
